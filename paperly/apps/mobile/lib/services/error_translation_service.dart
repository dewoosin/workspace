...
class ErrorTranslationService {
  static const Map<String, String> _errorMessages = {
    ...
    'ARTICLE.LOAD_FAILED': '글을 불러올 수 없습니다.',
    'ARTICLE.BOOKMARK_SUCCESS': '북마크에 추가되었습니다.',
    'ARTICLE.BOOKMARK_REMOVED': '북마크에서 제거되었습니다.',
    'ARTICLE.LIKE_SUCCESS': '좋아요를 눌렀습니다.',
    'ARTICLE.LIKE_REMOVED': '좋아요를 취소했습니다.',
    
    // 작가 팔로우 관련
    'FOLLOW.SUCCESS': '작가를 팔로우했습니다.',
    'FOLLOW.REMOVED': '팔로우를 취소했습니다.',
    'FOLLOW.FAILED': '팔로우에 실패했습니다.',
  ...
  static String translateFromError(dynamic error) {
    ...
    if (error.toString().contains('DioException')) {
      // 응답 데이터에서 에러 코드 추출 시도
      try {
        final response = error.response?.data;
        if (response is Map<String, dynamic>) {
          return translateFromResponse(response);
        }
      } catch (e) {
        // 파싱 실패 시 HTTP 상태 코드 기반 메시지 사용
      }
