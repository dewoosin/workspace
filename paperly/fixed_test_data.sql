-- 기존 스키마에 맞는 테스트 데이터 추가

SET search_path TO paperly, public;

-- 테스트 기사들 추가
INSERT INTO paperly.articles (
  id, title, slug, summary, subtitle, excerpt, content, 
  status, visibility, author_id, category_id, view_count, like_count,
...
(
  ...
  (SELECT id FROM paperly.categories LIMIT 1),
  127,
  23,
  NOW() - INTERVAL '2 days',
  NOW() - INTERVAL '2 days',
