...
-- A/B 테스트 노출 및 상호작용
CREATE TABLE IF NOT EXISTS paperly.ab_test_interactions (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    session_id UUID NOT NULL REFERENCES paperly.user_sessions(session_id) ON DELETE CASCADE,
    user_id UUID REFERENCES paperly.users(id) ON DELETE SET NULL,
    experiment_id UUID, -- ab_test_experiments 테이블과 연결
    
    -- 실험 정보
    experiment_name VARCHAR(100) NOT NULL,
    variant_name VARCHAR(50) NOT NULL, -- control, treatment_a, treatment_b, etc.
    
    -- 상호작용 정보
    interaction_type VARCHAR(50) NOT NULL, -- impression, click, conversion, etc.
    interaction_value DECIMAL(10,4), -- 측정값
    
    -- 컨텍스트
    page_url TEXT,
    element_tested VARCHAR(100), -- 테스트된 요소
